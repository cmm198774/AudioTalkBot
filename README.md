...
- 字幕可开关（用户语音转写 + 模型回复文本）
- 输出模式：语音 / 文字 / 语音+文字（"仅语音"对话中不显示字幕，
  但记录照常保存，切换会话回看历史仍能看到文字）
- system prompt 设置面板：保存即生效，支持人设预设保存与切换
- 多会话管理：新建 / 切换 / 删除 / 自动命名，切换旧会话自动恢复上下文
- 小黑板：板书通过 `write_to_board` 工具调用实现——工具参数只展示不朗读，
  模型可以先口头讲解、再写黑板、写完接着说，一轮内可多次穿插。
  黑板与聊天区比例可拖拽（默认 1:1），可开关、可清空
- 上下文管理：设置面板实时显示上下文用量，支持一键清空历史、
  一键压缩（用文本模型把旧对话总结成摘要，保留最近几条原样）
...
2. 配置 API key：复制 `.env.example` 为 `.env`，填入百炼 API key：
    ...
    上下文压缩默认用工作空间内的 `qwen3.6-flash` 生成摘要，
    可用 `.env` 的 `SUMMARY_MODEL` 覆盖（必须是该端点实际提供的模型）。
...
`docs/superpowers/specs/2026-09-03-blackboard-feature-design.md`（小黑板）
...
- **小黑板不写字**：板书由模型调用 `write_to_board` 工具写入，
  工具内容只上黑板、不会被朗读。触发语越明确越可靠
  （"把……写到黑板上 / 板书一下……"）。模型偶尔会选择口头讲解而不动用
  黑板，换个说法再试即可。黑板内容在结束对话、切换会话时自动清空；
  早期版本历史记录里的板书以 📋 气泡呈现。
...
- **SSL / 证书报错**：某些 Windows + conda 环境下默认证书加载会失败。
  本项目已在 `app/config.py` 的 `build_ssl_context()` 中显式用 `certifi`
  证书构造 SSL 上下文，所有对外 HTTPS / WSS 连接（模型 WebSocket、
  摘要压缩请求）统一使用它。若自行安装依赖遇到 `SSLEOFError`，
  优先使用 `python -m pip install --cert <certifi/cacert.pem>`。
